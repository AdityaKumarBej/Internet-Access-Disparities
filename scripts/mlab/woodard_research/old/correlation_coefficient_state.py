...
grouped = df_cleaned.groupby('state_name')

# Create an empty DataFrame to store the correlation coefficients, average download speed, and average household income
correlation_df = pd.DataFrame(columns=['state_name', 'income_download_correlation', 'avg_download_speed', 'avg_household_income'])
...
for state_name, group in grouped:
    ...
    correlation_df = correlation_df.append({'state_name': state_name,
...
correlation_df.to_csv('../../../results/mlab/US/Woodard_research/correlation_coefficients_by_state.csv', index=False)
