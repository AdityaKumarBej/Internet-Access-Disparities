...
# df = pd.read_csv('../../../../results/ookla/US/cbg/raw_masters/ookla_fixed_cbg_master_06_middlecalifornia.csv') 

df = pd.read_csv('../../../../results/ookla/US/cbg/raw_census/ookla_fixed_cbg_06_113_census.csv') 


speeds = ['avg_d_mbps', 'avg_u_mbps', 'avg_lat_ms']
attributes = ['population_density', 'median_household_income', 'percentage_female', 'percentage_age_18_64', 'percentage_age_over_65', 'less_than_high_school_diploma', 'high_school_diploma_only', 'bachelor_or_associate_degree' ,'masters_or_more', 'hispanic_latino', 'white_alone', 'black_or_african_american_alone', 'asian_alone']   # Replace with your actual attribute columns
...
sns.heatmap(correlation_matrix.rename(index=attributes_new, columns=speeds_new).T, 
            annot=True, cmap='coolwarm', vmin=-1, vmax=1, ax=axes[0])
axes[0].set_title('Correlation Coefficient Heatmap (Attributes vs Speeds)')

# Plot the heatmap for p-values on the second subplot (bottom)
sns.heatmap(p_values.rename(index=attributes_new, columns=speeds_new).T, 
            annot=False, cmap='coolwarm', vmin=0, vmax=0.1, ax=axes[1])
